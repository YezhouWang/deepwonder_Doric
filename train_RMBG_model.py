...
if __name__ == '__main__':
    ...
    for epoch in range(0, opt.n_epochs):
        ...
        for index in range(len(coor_list)):
            ...
            # --- robust name handling (WSL + Linux safe) ---
            # original name from coordinate list
            raw_name = per_coor['name']
            # normalize filename (remove extension if exists)
            base_name = raw_name.replace('.tif', '').replace('.tiff', '')
            # decide which one this refers to
            if 'w_bg' in base_name:
                input_name = base_name
                gt_name = base_name.replace('w_bg', 'wo_bg')
            else:
                gt_name = base_name
                input_name = base_name.replace('wo_bg', 'w_bg')
            # safety checks (fail early, fail loud)
            if input_name not in input_list:
                raise KeyError(f"Input key '{input_name}' not found. Available: {list(input_list.keys())}")
            if gt_name not in GT_list:
                raise KeyError(f"GT key '{gt_name}' not found. Available: {list(GT_list.keys())}")
            # fetch data
            input_im = input_list[input_name]
            GT_im = GT_list[gt_name]
            ...
            input = torch.from_numpy(np.expand_dims(np.expand_dims(input_patch, 3),0)).to(device)
            input = input.permute([0,4,1,2,3])
            target = torch.from_numpy(np.expand_dims(np.expand_dims(GT_patch, 3),0)).to(device)
            ...
            real_A = FFDrealign4(input).to(device)
            real_B = FFDrealign4(target).to(device)
